...
final class ChatSuggestionsInputNode: ChatInputNode {
    ...
    init(
        ...
        self.inputNodeInteraction = ChatBotsInputNodeInteraction(navigateToCollectionId: { [weak self] id in
            ...
            self?.controllerInteraction.handleSuggestionTap($0, $1, $2)
        }, buyBot: { [weak self] bot in
            //TODO: обновить доступность кнопки
            ...
            self.storeByUser = true
            ChatBotsManager.shared.enableBot(bot, enabled: enabled, userId: self.account.peerId.id, completion: {})
            self.updateStorePane(for: bot)

...
enum ChatListSelectionPeerId: Hashable {
    case peer(PeerId)
    case deviceContact(DeviceContactStableId)
}
...
final class ChatListSelectionNode: ASDisplayNode {
    ...
    init(account: Account, presentation: ChatListSelectionPresentation, filters: [ChatListSelectionFilter] = [.excludeSelf], selectionState: ChatListSelectionNodeGroupSelectionState? = nil) {
            ...
            transition = query
                |> mapToSignal { query in
                    ...
                    return combineLatest(foundLocalContacts, foundRemoteContacts, foundDeviceContacts, selectionStateSignal, themeAndStringsPromise.get())
                        |> mapToQueue { localPeersAndStatuses, remotePeers, deviceContacts, selectionState, themeAndStrings -> Signal<ChatListSelectionNodeTransition, NoError> in
                            let signal = deferred { () -> Signal<ChatListSelectionNodeTransition, NoError> in
                                ...
                                var peers: [ChatListSelectionPeer] = []
                                ...
                                let entries = ChatListSelectionNodeEntries(accountPeer: nil, peers: peers, presences: localPeersAndStatuses.1, presentation: presentation, selectionState: selectionState, theme: themeAndStrings.0, strings: themeAndStrings.1, dateTimeFormat: themeAndStrings.2, sortOrder: themeAndStrings.3, displayOrder: themeAndStrings.4, disabledPeerIds: disabledPeerIds)
                                let previous = previousEntries.swap(entries)
                                return .single(preparedChatListSelectionNodeTransition(account: account, from: previous ?? [], to: entries, interaction: interaction, firstTime: previous == nil, animated: false))
                            }
